<!--
  Marketplace view — unified screen (Packs / Templates / Bundled).

  Marketplace v2 (V2.5.0c): the catalog of both extensions (packs) and
  workflow templates is now served by license.knot.tools. Knot Core
  consumes everything via /api/marketplace.php (single round-trip) and
  uses /api/templates.php only for instantiation. Templates persist
  in llx_knot_template as a local cache mirror.

  Each row carries a `tier` (free / beta / pro / enterprise) controlled
  from the license.knot.tools admin backoffice. Pro Pack activation is
  intentionally NOT exposed before the Pro Pack module is installed —
  instead a clear instructions banner points to https://license.knot.tools.
...
async function load(opts?: { refresh?: boolean }): Promise<void> {
  ...
  try {
    ...
    error.value = err instanceof Error ? err.message : t('marketplace.loadFailed');
  } finally {
    loading.value = false;
...
async function instantiate(tmpl: MarketplaceTemplate): Promise<void> {
  ...
  try {
    ...
    error.value = err instanceof Error ? err.message : t('marketplace.instantiateFailed');
  } finally {
    creating.value = null;
  }
}

function lockedCta(tmpl: MarketplaceTemplate): { label: string; href: string } {
  const status = tmpl.lockedReason?.status ?? 'unknown';
  const slug = tmpl.tier === 'enterprise' ? KNOWN_SKU_ENTERPRISE : KNOWN_SKU_PRO_PACK;
  if (status === 'expired') {
    return {
      label: t('marketplace.lockedCtaRenew'),
      href: `https://license.knot.tools/checkout?product=${slug}`,
    };
  }
...
const filteredPacks = computed<MarketplacePack[]>(() => {
  const list = marketplace.value?.packs ?? [];
...
const templateCategories = computed<string[]>(() => {
  const set = new Set<string>();
  (marketplace.value?.templates ?? []).forEach((t) => {
    if (t.category) set.add(t.category);
  });
  return [...set].sort();
});
...
const filteredBundled = computed<BundledTemplateSummary[]>(() => {
  const q = search.value.trim().toLowerCase();
  return bundledTemplates.value.filter((t) => {
...
const filteredTemplates = computed<MarketplaceTemplate[]>(() => {
  const q = search.value.trim().toLowerCase();
  return (marketplace.value?.templates ?? []).filter((t) => {
    if (tierFilter.value && t.tier !== tierFilter.value) return false;
    if (templateCategoryFilter.value && t.category !== templateCategoryFilter.value) return false;
    if (!q) return true;
    return (
...
<template>
  <div class="k-p-6 k-max-w-[1280px] k-mx-auto k-space-y-5">
    <header class="k-flex k-items-start k-justify-between k-gap-4 k-flex-wrap">
      <div class="k-flex k-items-center k-gap-3">
        ...
        <div>
          <h1 class="k-text-2xl k-font-bold k-text-knot-text">{{ t('marketplace.title') }}</h1>
      ...
      <div class="k-flex k-items-center k-gap-2">
        <button
          ...
          class="k-px-3 k-py-2 k-text-sm k-text-knot-text-muted hover:k-text-knot-text k-flex k-items-center k-gap-1.5 k-border k-border-knot-border k-rounded-knot-sm disabled:k-opacity-50"
          :title="t('marketplace.refreshTitle')"
          @click="forceRefresh"
        >
          <RefreshCw :size="12" :class="refreshing ? 'k-animate-spin' : ''" />
          {{ refreshing ? t('marketplace.refreshBusy') : t('marketplace.refreshIdle') }}
        </button>
        <a
          href="https://knot.tools/pricing"
          target="_blank"
          rel="noopener"
          class="k-px-3 k-py-2 k-text-sm k-text-knot-text-muted hover:k-text-knot-text k-flex k-items-center k-gap-1.5 k-border k-border-knot-border k-rounded-knot-sm"
        >
    ...
    <nav class="k-border-b k-border-knot-border k-flex k-gap-1">
      <button
        type="button"
        :class="[
      ...
      >
        ...
        <span v-if="marketplace?.packs.length" class="k-text-[11px] k-font-normal k-bg-knot-surface-soft k-text-knot-text-muted k-px-1.5 k-py-0.5 k-rounded-knot-pill">
          {{ marketplace?.packs.length }}
        </span>
      </button>
      <button
        type="button"
      ...
      >
        ...
        <span v-if="marketplace?.templates.length" class="k-text-[11px] k-font-normal k-bg-knot-surface-soft k-text-knot-text-muted k-px-1.5 k-py-0.5 k-rounded-knot-pill">
    ...
    <div class="k-flex k-items-center k-gap-3 k-flex-wrap">
      <div class="k-relative k-flex-1 k-max-w-md">
        ...
        <input
          ...
          :placeholder="
            activeTab === 'templates'
              ? t('marketplace.searchTemplate')
              : activeTab === 'bundled'
                ? t('marketplace.searchBundled')
                : t('marketplace.searchPack')
          "
    ...
    <div
      v-if="marketplace?.packsMeta.stale"
    ...
    <div
      v-if="activeTab === 'templates' && marketplace?.templatesMeta.stale"
      class="k-bg-knot-warning-soft k-text-knot-warning k-px-4 k-py-2 k-rounded-knot-sm k-text-xs k-flex k-items-center k-gap-2"
    >
      <AlertTriangle :size="14" />
    ...
    <section v-if="activeTab === 'packs'">
      <div v-if="loading" class="k-flex k-items-center k-gap-2 k-text-knot-text-soft k-py-12 k-justify-center">
        <Loader2 :size="14" class="k-animate-spin" /> {{ t('marketplace.loadingPacks') }}
      </div>
      <div v-else-if="filteredPacks.length === 0" class="k-text-center k-py-12 k-text-knot-text-soft">
        {{ t('marketplace.noPacks') }}
      </div>
      <div v-else class="k-grid k-gap-4 sm:k-grid-cols-2 lg:k-grid-cols-3">
        ...
        >
          ...
          <!-- Action area: only show "Activer" once the module is locally
               installed. Otherwise show clear instructions pointing to
               license.knot.tools (the only place to buy a license). -->
          ...
          >
            ...
            <ol class="k-list-decimal k-list-inside k-space-y-1">
              <li>
                ...
                <a
                  :href="pack.buyUrl ?? `https://license.knot.tools/checkout?product=${pack.slug}`"
                  target="_blank"
                  rel="noopener"
                  class="k-text-knot-primary hover:k-underline"
                >license.knot.tools</a>
    ...
    <section v-if="activeTab === 'templates'">
      ...
      <div v-else-if="filteredTemplates.length === 0" class="k-text-center k-py-12 k-text-knot-text-soft">
        {{ t('marketplace.noTemplatesMatch') }}
      </div>
      <div v-else class="k-grid k-gap-4 sm:k-grid-cols-2 lg:k-grid-cols-3">
        <article
    ...
    <section v-if="activeTab === 'bundled'" class="k-space-y-3">
      ...
      <div v-else-if="filteredBundled.length === 0" class="k-text-center k-py-12 k-text-knot-text-soft">
        {{ t('marketplace.noBundledMatch') }}
      </div>
      <div v-else class="k-grid k-gap-4 sm:k-grid-cols-2 lg:k-grid-cols-3">
        <article
