...
- Erreurs credentials
- **Updates apply (`specs/updates-apply-suite.spec.ts`)** — cartes `updates-card-{slug}` + confirm `KConfirmDialog` + POST `updates_apply.php` pour **`knot`**, **`knot-pro-pack`**, **`knot-migration`** (helper `tests/e2e/helpers/updates-apply.ts`). Skip si déjà à jour / Apply désactivé. Smoke liste : `updates-smoke.spec.ts`.
